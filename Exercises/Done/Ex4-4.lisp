...
(defun bst-elements (tree)
  (let ((biggest (bst-max tree)))
    (if (null biggest)
        nil
      (cons (node-elt biggest) (bst-elements (bst-remove (node-elt biggest) tree #'<))))))
|#
...
(defun bst-elements (tree)
  (if (null tree)
      nil
    (append (bst-elements (node-r tree)) (list (node-elt tree)) (bst-elements (node-l tree)))))


(defun bst-elements (tree)
  (let ((result nil))
    (bst-traverse #'(lambda (x) (push x result)) tree)
    result))

#|
(defun bst-elements (tree)
  (do ((smaller-tree tree (bst-remove (node-elt (bst-max smaller-tree)) smaller-tree #'<))
       (lst nil (cons (node-elt (bst-max smaller-tree)) lst)))
      ((null smaller-tree) (reverse lst))))
|#